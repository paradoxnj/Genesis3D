	...
	const char *	FileSpec)
{
	...
	File = static_cast<VFSFile*>(Handle);
	...
	Finder = static_cast<VFSFinder*>(geRam_Allocate(sizeof(*Finder)));
	if	(!Finder)
		return NULL;
...
static	geBoolean	GENESISCC FSVFS_FinderGetNextFile(void *Handle)
{
	...
	Finder = static_cast<VFSFinder*>(Handle);
	...
	unsigned int 	OpenModeFlags)
{
	...
	Context = static_cast<VFSFile*>(Handle);
	...
	*/
	...
	NewFile = static_cast<VFSFile*>(geRam_Allocate(sizeof(*NewFile)));
	if	(!NewFile)
		return NewFile;
...
//#pragma todo  ("FSVFS_OpenNewSystem: READ/WRITE opens not supported")
		...
		NewFS = static_cast<VFSFile*>(geRam_Allocate(sizeof(*NewFS)));
		if	(!NewFS)
			return NewFS;
...
static	int			ClampOperationSize(const VFSFile *File, int Size)
{
	assert(!File->Directory);
	assert(File->CurrentRelPos >= 0);
	return std::min<int>(File->Length - File->CurrentRelPos, Size);
}
...
static	geBoolean	GENESISCC FSVFS_EOF(const void *Handle)
{
	...
	File = static_cast<const VFSFile*>(Handle);
