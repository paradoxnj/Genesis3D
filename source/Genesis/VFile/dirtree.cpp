...
static	char *	DuplicateString(const char *String)
{
	...
	NewString = static_cast<char*>(geRam_Allocate(Length));
	if	(NewString)
...
DirTree *DirTree_Create(void)
{
	...
	Tree = static_cast<DirTree*>(geRam_Allocate(sizeof(*Tree)));
	if	(!Tree)
		return Tree;
...
static	geBoolean	ReadTree(geVFile *File, DirTree **TreePtr)
{
	...
	assert(Length > 0);
	Tree->Name = static_cast<char*>(geRam_Allocate(Length));
	if	(!Tree->Name)
	{
		geRam_Free(Tree);
		return GE_FALSE;
...
DirTree * DirTree_AddFile(DirTree *Tree, const char *Path, geBoolean IsDirectory)
{
	...
	NewEntry = static_cast<DirTree*>(geRam_Allocate(sizeof(*NewEntry)));
	if	(!NewEntry)
		return NULL;
...
DirTree_Finder * DirTree_CreateFinder(DirTree *Tree, const char *Path)
{
	...
	Finder = static_cast<DirTree_Finder*>(geRam_Allocate(sizeof(*Finder)));
	if	(!Finder)
		return Finder;
