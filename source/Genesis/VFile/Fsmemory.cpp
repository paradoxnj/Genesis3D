	...
	unsigned int 	OpenModeFlags)
{
	...
	MemContext = static_cast<geVFile_MemoryContext*>(Context);
	...
	NewFS = static_cast<MemoryFile*>(geRam_Allocate(sizeof(*NewFS)));
	if	(!NewFS)
		return NewFS;
	memset(NewFS, 0, sizeof(*NewFS));

	NewFS->Memory = static_cast<char*>(MemContext->Data);
	...
	int 			ContextSize)
{
	...
	File = static_cast<MemoryFile*>(Handle);
	...
	MemoryContext = static_cast<geVFile_MemoryContext*>(Context);
...
static	geBoolean	GENESISCC TestForExpansion(MemoryFile *File, int Size)
{
	...
	if	(File->AllocatedSize - File->Position < Size)
	{
		...
		NewBlock = static_cast<char*>(geRam_Realloc(File->Memory, NewSize));
		if	(!NewBlock)
			return GE_FALSE;
...
static	geBoolean	GENESISCC FSMemory_EOF(const void *Handle)
{
	...
	File = static_cast<const MemoryFile*>(Handle);
