	...
	const char *	FileSpec)
{
	...
	File = static_cast<DosFile*>(Handle);

	CHECK_HANDLE(File);

	Finder = static_cast<DosFinder*>(geRam_Allocate(sizeof(*Finder)));
	if	(!Finder)
		return NULL;
...
static	geBoolean	GENESISCC FSDos_FinderGetNextFile(void *Handle)
{
	...
	Finder = static_cast<DosFinder*>(Handle);
	...
	unsigned int 	OpenModeFlags)
{
	...
	DosFS = static_cast<DosFile*>(Handle);

	if	(DosFS && DosFS->IsDirectory != GE_TRUE)
		return NULL;

	NewFile = static_cast<DosFile*>(geRam_Allocate(sizeof(*NewFile)));
	if	(!NewFile)
		return NewFile;
	...
	Length = strlen(Buff);
	NewFile->FullPath = static_cast<char*>(geRam_Allocate(Length + 1));
	if	(!NewFile->FullPath)
		goto fail;
...
static	geBoolean	GENESISCC FSDos_EOF(const void *Handle)
{
	...
	File = static_cast<const DosFile*>(Handle);
