...
static	geBoolean GENESISCC geVFile_RegisterFileSystemInternal(const geVFile_SystemAPIs *APIs, geVFile_TypeIdentifier *Type)
{
	...
	NewList = static_cast<geVFile_SystemAPIs**>(geRam_Realloc(RegisteredAPIs, sizeof(*RegisteredAPIs) * (SystemCount + 1)));
	if	(!NewList)
		return GE_FALSE;
	...
	unsigned int 			OpenModeFlags)
{
	...
	File = static_cast<geVFile*>(geRam_Allocate(sizeof(*File)));
	if	(!File)
	{
		if	(BaseFile)
			geVFile_Close(BaseFile);
	...
	File->APIs = 		APIs;
	File->FSData = 		FSData;
	File->SearchList = 	static_cast<FSSearchList*>(geRam_Allocate(sizeof(*File->SearchList)));
...
static	FSSearchList *	CopySearchList(const FSSearchList *SearchList)
{
	...
	while	(SearchList)
	{
		...
		Temp = static_cast<FSSearchList*>(geRam_Allocate(sizeof(*Tail)));
		if	(!Temp)
		{
...
{
	FSSearchList *	SearchList = nullptr;
	...
	const char *FileSpec)
{
	...
	Finder = static_cast<geVFile_Finder*>(geRam_Allocate(sizeof(*Finder)));
	if	(!Finder)
		return Finder;
