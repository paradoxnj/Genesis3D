	...
	
{
	...
	P = static_cast<gePath*>(geRam_Allocate(sizeof(gePath)));
...
GENESISAPI gePath *GENESISCC gePath_CreateCopy(const gePath *Src)
{
	...
	int i,Count;
	gePath_Interpolator RInterp=GE_PATH_INTERPOLATE_LINEAR;
	gePath_Interpolator TInterp=GE_PATH_INTERPOLATE_LINEAR;
...
geBoolean GENESISCC gePath_ReadChannel_F0_(int ChannelMask, gePath_Channel *C, geVFile* pFile)
{
	...
	while(NumItemsRead < NumItemsNeeded)
	{
		...
		if(_strnicmp(line, GE_PATH_CHANNEL_INTERPOLATE_ID, sizeof(GE_PATH_CHANNEL_INTERPOLATE_ID)-1) == 0)
		{
		...
		else if(_strnicmp(line, GE_PATH_CHANNEL_STARTTIME_ID, sizeof(GE_PATH_CHANNEL_STARTTIME_ID)-1) == 0)
		{
		...
		else if(_strnicmp(line, GE_PATH_CHANNEL_ENDTIME_ID, sizeof(GE_PATH_CHANNEL_ENDTIME_ID)-1) == 0)
		{
		...
		else if(_strnicmp(line, GE_PATH_CHANNEL_KEYLIST_ID, sizeof(GE_PATH_CHANNEL_KEYLIST_ID)-1) == 0)
		{
...
gePath* GENESISCC gePath_CreateFromFile_F0_(geVFile* pFile)
{
	...
	P = gePath_Create(GE_PATH_INTERPOLATE_LINEAR, GE_PATH_INTERPOLATE_LINEAR, GE_FALSE);
	if( P == NULL )
		{
	...
	if(geVFile_GetS(pFile, line, LINE_LENGTH) == GE_FALSE)
		EXIT_ERROR
	if(_strnicmp(line, GE_PATH_LOOPED_ID, sizeof(GE_PATH_LOOPED_ID)-1) != 0)
		EXIT_ERROR
	...
	if(!geVFile_GetS(pFile, line, LINE_LENGTH) == GE_FALSE)
		EXIT_ERROR
	if(_strnicmp(line, GE_PATH_ROTATION_ID, sizeof(GE_PATH_ROTATION_ID)-1) != 0)
		EXIT_ERROR
	...
	if(geVFile_GetS(pFile, line, LINE_LENGTH) == GE_FALSE)
		EXIT_ERROR
	if(_strnicmp(line, GE_PATH_TRANSLATION_ID, sizeof(GE_PATH_TRANSLATION_ID)-1) != 0)
		EXIT_ERROR
...

	{
		...
		if (flag!=GE_FALSE)
			if (geQKFrame_WriteToFile( pFile, P->Rotation.KeyList, 
										gePath_PathToQKInterpolation((gePath_InterpolationType)P->Rotation.InterpolationType),
										Looped)==GE_FALSE)
				{
					return GE_FALSE;
				}
		...
		if (flag!=GE_FALSE)
			if (geVKFrame_WriteToFile( pFile, P->Translation.KeyList, 
										gePath_PathToVKInterpolation((gePath_InterpolationType)P->Translation.InterpolationType),
										Looped)==GE_FALSE)
				{
					return GE_FALSE;
				}
