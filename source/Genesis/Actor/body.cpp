	...
	if (B->XSkinVertexCount>0)
		{
			...
			B->XSkinVertexArray = static_cast<geBody_XSkinVertex*>(geRam_Allocate(u));
			if (B->XSkinVertexArray == NULL)
	...
	if (B->SkinNormalCount>0)
		{
			...
			B->SkinNormalArray = static_cast<geBody_Normal*>(geRam_Allocate(u));
			if (B->SkinNormalArray == NULL)
				{	geErrorLog_Add( ERR_BODY_ENOMEM , NULL);   return GE_FALSE;  }
	...
	if (B->BoneCount>0)
		{
			...
			B->BoneArray = static_cast<geBody_Bone*>(geRam_Allocate(u));
			if (B->BoneArray == NULL)
				{	geErrorLog_Add( ERR_BODY_ENOMEM , NULL);   return GE_FALSE;  }
	...
	if (B->MaterialCount>0)
		{
			...
			B->MaterialArray = static_cast<geBody_Material*>(geRam_Allocate(u));
			if (B->MaterialArray == NULL)
				{	geErrorLog_Add( ERR_BODY_ENOMEM , NULL);   return GE_FALSE;  }
			...
			if (u>0)
				{
					...
					B->SkinFaces[i].FaceArray = static_cast<geBody_Triangle*>(geRam_Allocate(u));
					if (B->SkinFaces[i].FaceArray == NULL)
						{	geErrorLog_Add( ERR_BODY_ENOMEM , NULL);   return GE_FALSE;  }
