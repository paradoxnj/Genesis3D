...
GENESISAPI geBoolean GENESISCC geMotion_SetName(geMotion *M, const char *Name)
{
	...
	NewName = static_cast<char*>(geRam_Allocate( strlen(Name)+1 ));
	if (NewName == NULL )
		{
	...
	gePath *P,const char *Name,int *PathIndex)
{
...

	{
		...
		NewPathArray = static_cast<gePath**>(geRam_Realloc(M->Leaf.PathArray, (1+PathCount) * sizeof(gePath*) ));
...

{
	...
	NewMixerArray = static_cast<geMotion_Mixer*>(geRam_Realloc(ParentMotion->Branch.MixerArray, (1+Count) * sizeof(geMotion_Mixer) ));
...
GENESISAPI geMotion * GENESISCC geMotion_RemoveSubMotion(geMotion *ParentMotion, int SubMotionIndex)
{
...

	{
		...
		if (ParentMotion->Branch.MixerCount == 0)
			...
			{
				NewMixerArray = static_cast<geMotion_Mixer*>(geRam_Realloc(ParentMotion->Branch.MixerArray, 
...
GENESISAPI geMotion* GENESISCC geMotion_CreateFromFile(geVFile* pFile)
{
	...
	if(u == MOTION_ASCII_FILE_TYPE)
	{
		...
		while(NumItemsRead < NumItemsNeeded)
		{
			...
			else if(_strnicmp(line, MOTION_PATHCOUNT_ID, sizeof(MOTION_PATHCOUNT_ID)-1) == 0)
			{
			...
			else if(_strnicmp(line, MOTION_EVENTS_ID, sizeof(MOTION_EVENTS_ID)-1) == 0)
			{
				int flag;
			...
			else if(_strnicmp(line, MOTION_NAMEARRAY_ID, sizeof(MOTION_NAMEARRAY_ID)-1) == 0)
			{
				int flag;
			...
			else if(_strnicmp(line, MOTION_PATHARRAY_ID, sizeof(MOTION_PATHARRAY_ID)-1) == 0)
			{
				int i,count;
				...
				M->Leaf.PathArray = static_cast<gePath**>(geRam_Allocate( count * sizeof(gePath*) ));
...
static geBoolean GENESISCC geMotion_ReadBinaryLeaf(geMotion *M, geVFile *pFile)
{
	...
	M->Leaf.PathCount = 0;
	M->Leaf.PathArray = static_cast<gePath**>(geRam_Allocate( Header.PathCount * sizeof(gePath*) ));
...
static geMotion *GENESISCC geMotion_CreateFromBinaryFile(geVFile *pFile)
{
	...
	if (NameLength>0)
		{
			M->Name = static_cast<char*>(geRam_Allocate(NameLength));
			if ( M->Name == NULL )
				{
					geErrorLog_Add(ERR_MOTION_CREATE_ENOMEM, NULL);
					geMotion_Destroy(&M);
