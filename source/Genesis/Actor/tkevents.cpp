...
geBoolean GENESISCC geTKEvents_Insert(geTKEvents* pEvents, geTKEvents_TimeType tKey, const char* pEventData)
{
	...
	pKeyInfo = static_cast<EventType*>(geTKArray_Element(pEvents->pTimeKeys, nIndex));
	...
	pNewData = static_cast<char*>(geRam_Realloc(pEvents->pEventData, pEvents->DataSize + DataLength));
	if(!pNewData)
	{
	...
	{
		EventType* pNextKeyInfo = static_cast<EventType*>(geTKArray_Element(pEvents->pTimeKeys, nIndex + 1));
...
geBoolean GENESISCC geTKEvents_Delete(geTKEvents* pEvents, geTKEvents_TimeType tKey)
{
	...
	if(nIndex < geTKArray_NumElements(pEvents->pTimeKeys) - 1)
	{
		...
		pKeyInfo = static_cast<EventType*>(geTKArray_Element(pEvents->pTimeKeys, nIndex + 1));
	...
	{
		pNewData = static_cast<char*>(geRam_Realloc(pEvents->pEventData, pEvents->DataSize));
...
{											\
	...
	return NULL;							\
		...
		if(_strnicmp(line, TKEVENTS_TIMEKEYS_ID, sizeof(TKEVENTS_TIMEKEYS_ID)-1) != 0)
			ABORT_READ_AND_FREE;
		...
		pEvents->pEventData = static_cast<char*>(geRam_Allocate(pEvents->DataSize));
		if(!pEvents->pEventData)
			ABORT_READ_AND_FREE_ALL;

		// The strings are read in with a CR.  The max line length will then be
		// the datasize + 1.
		pTextLine = static_cast<char*>(geRam_Allocate(pEvents->DataSize + 1));
		if(pTextLine == NULL)
			ABORT_READ_AND_FREE_ALL;
		...
		while(i > 0)
		{
			...
			if(!geTKArray_Insert(&pEvents->pTimeKeys, Time, (int*)&u))
				ABORT_READ_AND_FREE_ALL;
			pEInfo = static_cast<EventType*>(geTKArray_Element(pEvents->pTimeKeys, u));
			pEInfo->DataOffset = v;
	...
	// Writes the array to the given stream.
{
	...
	for(i=0;i<NumElements;i++)
		{
			pEInfo = static_cast<EventType*>(geTKArray_Element(pEvents->pTimeKeys, i));
	...
	if(Index < geTKArray_NumElements(pTimeKeys))
	{
		*pTime = geTKArray_ElementTime(pTimeKeys, Index);
		if(*pTime + GE_TKA_TIME_TOLERANCE < pTKEI->EndTime)
		{
			...
			pKeyInfo = static_cast<EventType*>(geTKArray_Element(pTimeKeys, Index));
