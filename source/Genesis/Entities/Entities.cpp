...
static geBoolean InsertEntityInClassList(geWorld *World, geEntity *Entity)
{
	...
	for (i=0; i< World->NumEntClassSets; i++)
	{
		...
		if (!_stricmp(WSet[i].ClassName, EntClassName))
		{
...
geBoolean Ent_WorldInit(geWorld *World)
{
	...
	EntitySet = LoadEntitySet((const char*)BSP->GFXEntData, BSP->NumGFXEntData);
...
static char *CopyString(const char *String)
{
	...
	NewString = static_cast<char*>(geRam_Allocate(strlen(String)+1));
...
geEntity *geEntity_Create(void)
{
	...
	Entity = static_cast<geEntity*>(geRam_Allocate(sizeof(geEntity)));
...
geEntity_Epair *geEntity_EpairCreate(void)
{
	...
	Epair = static_cast<geEntity_Epair*>(geRam_Allocate(sizeof(geEntity_Epair)));
...
geEntity_EntitySet *geEntity_EntitySetCreate(void)
{
	...
	EntitySet = static_cast<geEntity_EntitySet*>(geRam_Allocate(sizeof(geEntity_EntitySet)));
...
static geBoolean BuildClassTypes(geEntity_EntitySet *EntitySet)
{
	...
	for (Set = EntitySet; Set; Set = Set->Next)
	{
		...
		if (_stricmp(Name, "%typedef%"))	// No %typedef% info
			continue;
	...
	for (Set = EntitySet; Set; Set = Set->Next)
	{
		...
		if (_stricmp(Name, "%typedef%"))	// Not a typedef
			continue;
		...
		for (Epair = Entity->Epairs; Epair; Epair = Epair->Next)
		{
			...
			if	(!_stricmp(Epair->Key, "classname"))
				continue;
			if	(!_stricmp(Epair->Key, "%typename%"))
				continue;

			assert(_stricmp(Epair->Key, "%defaultvalue%"));
...
static geBoolean ParseClassUserData(geEntity_EntitySet *EntitySet, geEntity *Entity)
{
	...
	for (Epair = Entity->Epairs; Epair; Epair = Epair->Next)
	{
		...
		UData = static_cast<char*>(Entity->UserData);
		...
		switch (Field->TypeClass->Type)
		{
			...
			case TYPE_INT:
				sscanf(Epair->Value, "%d", (int*)UData + Field->Offset);
				break;
