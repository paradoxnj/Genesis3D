...
if ( (Ret = static_cast<MemPool*>(RamCalloc(sizeof(MemPool)))) == NULL )
  return(NULL);
...
if ( (Ret->FreedHunks = static_cast<void**>(RamCalloc(Ret->MaxNumFreedHunks*sizeof(void *)))) == NULL )
{
	RamFree(Ret);
	return(NULL);
}
...
MemBlock * NewMemBlock;
	...
	if ( (NewMemBlock = static_cast<MemBlock*>(RamCalloc(sizeof(MemBlock)))) == NULL )
		return(0);

	NewMemBlock->MemLength = NumHunks * Pool->HunkLength;
	NewMemBlock->MemFree = NewMemBlock->MemLength;

	if ( (NewMemBlock->MemBase = static_cast<char*>(RamCalloc(NewMemBlock->MemLength))) == NULL )
	{
		RamFree(NewMemBlock);
		return(0);
	}
...
int MemPool_FreeHunk(MemPool * Pool,void * Hunk)
{
	...
	if ( Pool->NumFreedHunks >= Pool->MaxNumFreedHunks )
	{
		...
		Pool->FreedHunks = static_cast<void**>(RamRealloc(Pool->FreedHunks,Pool->MaxNumFreedHunks*sizeof(void *)));
