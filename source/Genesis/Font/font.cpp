...
GENESISAPI void GENESISCC geFont_Destroy(geFont **font)
{
   ...
   while (curFontBitmap)
   {
      tempFontBitmap = static_cast<geFontBitmap*>(curFontBitmap->next);
                 ...
                 uint32 asciiValue, unsigned char *cellBuffer, DWORD bufferSize)
{
   ...
   win32OldFont = (HFONT)SelectObject( hdc, win32Font); 
...
//   bufferWidth = glyphMetrics.gmBlackBoxX;
   ...
   destBits = static_cast<unsigned char*>(geBitmap_GetBits(lock));
                                                  ...
                                                  )
{
   ...
   for (asciiValue = leastIndex; asciiValue <= mostIndex; asciiValue++)
   {
      ...
      while (fontBitmap)
      {
         ...
         fontBitmap = static_cast<geFontBitmap*>(fontBitmap->next);
      }
                                                   ...
                                                   const GE_RGBA *Color, uint32 flags)
{
   ...
   oldFont = (HFONT)SelectObject( drawDC, winFont); 
   oldMap  = (HBITMAP)SelectObject( drawDC, hDIB); 
...
GENESISAPI int32 GENESISCC geFont_GetStringPixelWidth (geFont *font, const char *textString)
{
   ...
   else
   {
      ...
      oldFont = (HFONT)SelectObject( hdc, winFont); 
