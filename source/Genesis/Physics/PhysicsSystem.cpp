...
GENESISAPI geBoolean	GENESISCC gePhysicsSystem_AddObject(gePhysicsSystem *PS, gePhysicsObject *Object)
{
	...
	NewList = static_cast<gePhysicsObject**>(geRam_Realloc(PS->Objects, sizeof(*NewList) * (PS->PhysicsObjectCount + 1)));
	if	(!NewList)
		return GE_FALSE;
...
GENESISAPI geBoolean	GENESISCC gePhysicsSystem_AddJoint(gePhysicsSystem *PS, gePhysicsJoint *Joint)
{
	...
	NewList = static_cast<gePhysicsJoint**>(geRam_Realloc(PS->Joints, sizeof(*NewList) * (PS->PhysicsJointCount + 1)));
	if	(!NewList)
		return GE_FALSE;
