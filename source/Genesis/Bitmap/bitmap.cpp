...
geBitmap * Bmp;
	...
	Bmp = static_cast<geBitmap*>(MemPool_GetHunk(BitmapPool));
...
geRDriver_PixelFormat **pDriverFormatsPtr;
	pDriverFormatsPtr = static_cast<geRDriver_PixelFormat**>(Context);
...
//			Bmp->SeekMipCount = max(Bmp->SeekMipCount,4);
		...
		NumMipLevels = std::max<int>(Bmp->SeekMipCount,Bmp->Info.MaximumMip + 1);
...
geBoolean Ret = GE_TRUE;
	...
	Bmp->DriverInfo.ColorKey = Bmp->DriverInfo.HasColorKey;
	Bmp->DriverInfo.Format = GE_PIXELFORMAT_NO_DATA;
	Bmp->DriverInfo.Palette = NULL;
	Bmp->DriverMipLock = 0;
	Bmp->DriverBitsLocked = GE_FALSE;
...
//		return GE_FALSE;
	...
	Bmp->Info.MaximumMip = std::max<int>(Bmp->Info.MaximumMip,to);
...
int fmxtra,tow,toh,toxtra,fmw,fmh,fmstep,x,y,bpp;
	...
	else
	{
		...
		switch( bpp )
		{
			...
			{
				uint16 *fmp,*top;
				fmp = static_cast<uint16*>(FmBits);
				top = static_cast<uint16*>(ToBits);
				for(y=toh;y--;)
				{
					for(x=tow;x--;)
					{
			...
			{
				uint32 *fmp,*top;
				fmp = static_cast<uint32*>(FmBits);
				top = static_cast<uint32*>(ToBits);
				for(y=toh;y--;)
				{
					for(x=tow;x--;)
					{
...
//		return GE_FALSE;
	...
	Bmp->Info.MinimumMip = std::min<int>(Bmp->Info.MinimumMip,low);
	Bmp->Info.MaximumMip = std::max<int>(Bmp->Info.MaximumMip,high);
...
//		{} can't check this, cuz we use _BlitMip to create locks for read
	...
	if ( ! (SrcBits = static_cast<uint8*>(geBitmap_GetBits(SrcLock))) || 
		 ! (DstBits = static_cast<uint8*>(geBitmap_GetBits(DstLock))) )
	{
	...
	if ( SizeX < 0 )
		SizeX = std::min<int>(SrcLockInfo->Width,DstLockInfo->Width);
	if ( SizeY < 0 )
		SizeY = std::min<int>(SrcLockInfo->Height,DstLockInfo->Height);
							...
							const geBitmap_Palette *Palette )
{
	...
	DRV_Driver * Driver;
		...
		OldMaxMips = std::max<int>(Bmp->Info.MaximumMip,Bmp->DriverInfo.MaximumMip);
...
*/
		...
		Bmp->DriverInfo.Palette != Palette )
	{
		...
		{
			if ( ! geBitmap_AllocPalette(Bmp,GE_PIXELFORMAT_NO_DATA,Bmp->Driver) )
				return GE_FALSE;
...
{
	if ( ! Bmp ) return GE_PIXELFORMAT_NO_DATA;
	...
	else
	{
		for( mip = Bmp->Info.MinimumMip; mip <= Bmp->Info.MaximumMip; mip++ )
		{
			...
			uint32 MipDataLen;
				...
				if ( Bmp->Info.Stride == Bmp->Info.Width )
				{
					MipData = static_cast<uint8*>(Bmp->Data[mip]);
				...
				uint8 * fptr,*tptr;
					...
					if ( ! (MipData = static_cast<uint8*>(geRam_Allocate(MipDataLen)) ) )
					{
					...
					fptr = static_cast<uint8*>(Bmp->Data[mip]);
					tptr = MipData;
					for(y=h;y--;)
					{
						memcpy(tptr,fptr,w);
...
int bPad,myRowWidth,bmpRowWidth,pelBytes;
	...
	int y;
	char * row;
		row = static_cast<char*>(Bmp->Data[0]);
...
geBitmap_Info * pi;
	...
	pi->Format = static_cast<gePixelFormat>(data[1]); // could go in 5 bits
...
int SrcCKi = 0,DstCKi = 0;
	...
	SrcPtr = const_cast<uint8*>(static_cast<const uint8*>(SrcData));
	DstPtr = static_cast<uint8*>(DstData);
...
const gePixelFormat_Operations * ops;
	...
	//allocate(P);
	P = static_cast<geBitmap_Palette*>(geRam_AllocateClear(sizeof(geBitmap_Palette)));
	if ( ! P ) return NULL;
	//clear(P);
...
geRDriver_THandleInfo TInfo;
	...
	P->DriverHandle = geBitmap_CreateTHandle(Driver,Size,1,1,
			Format,GE_PIXELFORMAT_NO_DATA,GE_PIXELFORMAT_NO_DATA,gePixelFormat_HasAlpha(Format),0,RDRIVER_PF_PALETTE);
	if ( ! P->DriverHandle )
	{
	...
	if ( P->LockCount == 0 )
	{
		if ( P->HasColorKey )
		{
			...
			gePixelFormat_PixelGetter	GetPixel;
				...
				if ( P->Data )
				{
					Bits = static_cast<uint8*>(P->Data);
				}
				else if ( P->DriverBits )
				{
					Bits = static_cast<uint8*>(P->DriverBits);
				}
...
GENESISAPI geBoolean GENESISCC geBitmap_Palette_SetEntry(geBitmap_Palette *P,int Color,uint32 Pixel)
{
	...
	uint8 *Data;
		...
		Data = static_cast<uint8*>(P->Data) + Color * gePixelFormat_BytesPerPel(P->Format);
...
uint8 flags,b;
	...
	Format = (gePixelFormat)(flags & PALETTE_INFO_FORMAT_MASK);
...
GENESISAPI geBoolean GENESISCC geBitmap_GetAverageColor(const geBitmap *Bmp,int *pR,int *pG,int *pB)
{
	...
	int R,G,B,A,Rt,Gt,Bt,cnt,ck;
		...
		ptr = static_cast<uint8*>(Bmp->Data[0]);	
