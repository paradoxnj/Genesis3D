...
geBitmap_Palette * Pal;
	...
	root = static_cast<octNode*>(MemPool_GetHunk(octNodePool));
	assert(root);
	nLeaves = createOctTree(root,Info,Bits,GE_FALSE);

	leaves = static_cast<octNode**>(geRam_AllocateClear(sizeof(octNode)*nLeaves));
	assert(leaves);
...
geBitmap_Palette * Pal;
	...
	radix = static_cast<octNode*>(geRam_AllocateClear(sizeof(octNode)*RADIX_SIZE));
	assert(radix);
...
int distance,minDistance;
	...
	for(minDistance=256;minDistance>=0 && palGot < palEntries;minDistance>>=1)
	{
		...
		for(i=0;i<gotLeaves;i++)
		{
			...
			B = (*leavesPtr)->B;
			leavesPtr++;
			distance = findClosest(R,G,B,palette,palGot,nullptr);
...
//	pushTSC();
	...
	if ( doYUV )
	{
		switch(bpp)
		{
			...
			{
			const uint8 *ptr;
				ptr = static_cast<const uint8*>(Bits);
				for(y=h;y--;)
				{
					for(x=w;x--;)
					{
			...
			const uint16 *ptr;
			int R,G,B,A;
				ptr = static_cast<const uint16*>(Bits);
				for(y=h;y--;)
				{
					for(x=w;x--;)
					{
			...
			const uint32 *ptr;
			int R,G,B,A;
				ptr = static_cast<const uint32*>(Bits);
				for(y=h;y--;)
				{
					for(x=w;x--;)
					{
