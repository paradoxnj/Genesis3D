...
palOptInfo optInfo[256];
	...
	PixelOps = gePixelFormat_GetOperations(Info->Format);
	GetColor = PixelOps->GetColor;
	ptrEnd = (uint8 *)Bits + Info->Stride * Info->Height * PixelOps->BytesPerPel;
	...
	for(;;)
	{
		...
		if ( abs((int)(mse - last_mse)) < 50 && extraStepSize == 0 )
		{
			break;
		}
	}
